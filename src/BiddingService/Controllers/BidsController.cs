...
public class BidsController : ControllerBase
{
    ...
    public async Task<ActionResult<BidDto>> PlaceBid(string auctionId, int amount)
    {
        ...
        if (auction == null)
        {
            ...
            if (auction == null)
            {
                return BadRequest("Cannot accept bid on this auction, it does not exist");
            }
        }
        ...
        var bid = new Bid
        {
            Amount = amount,
            AuctionId = auctionId,
            Bidder = User.Identity?.Name ?? throw new InvalidOperationException("User identity is not set"),
        };
